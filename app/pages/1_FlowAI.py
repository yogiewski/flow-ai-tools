...
with st.sidebar:
    ...
    if selected_prompt_id != st.session_state.current_prompt:
        st.session_state.current_prompt = selected_prompt_id
        if selected_prompt_id:
            prompt_data = prompts_repo.get_prompt(selected_prompt_id)
            if prompt_data:
                st.markdown(f"**Using:** {prompt_data['title']}")
                st.markdown(f"*{prompt_data['content'][:200]}...*" if len(prompt_data['content']) > 200 else f"*{prompt_data['content']}*")
            else:
                st.markdown("*Tool not found*")
        else:
            st.markdown("*No tool selected - general chat*")
...
st.header(translator.get("chat_title"))
